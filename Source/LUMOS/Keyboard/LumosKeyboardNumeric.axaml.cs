...
{
    public partial class LumosKeyboardNumeric : UserControl, ISoftKeyboard
    {
        private IKeyboardListener? _keyboardListener;

        public LumosKeyboardNumeric()
        {
            InitializeComponent();
        }
        ...
        public void Resize(double baseKeyWidth)
        {
            foreach (StackPanel keyRow in this.Keys.Children)
            {
                foreach (Button button in keyRow.Children)
                {
                    ...
                    if (button.Name == Constants.KeyHide)
                    {
        ...
        private void OnKeyPress(object sender, RoutedEventArgs e)
        {
            ...
            {
                this._keyboardListener?.PressedKey(button.Name, button.Content.ToString(), KeyboardTypes.Numeric);
            }
        }
    }
}
